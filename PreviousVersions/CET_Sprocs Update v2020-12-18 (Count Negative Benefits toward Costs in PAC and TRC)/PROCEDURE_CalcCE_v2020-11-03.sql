...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.CalcCE_20201103'))
   exec('CREATE PROCEDURE [dbo].[CalcCE_20201103] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[CalcCE_20201103]
    @JobID INT = -1,
    @MEBens FLOAT = NULL,
    @MECost FLOAT = NULL,
    @FirstYear INT = 2013,
        ...
        ,ISNULL(
            SUM(
                ...
                (
                    ...
                    -- Remove negative savings from benefits from fuel substitution measures:
                    CASE
                        WHEN e.kWh2 < 0 OR ec.MeasImpactType LIKE '%FuelSub'
                        THEN 0
                        ELSE
...
AS (
    SELECT PrgID
        ,SUM(ElecBen) AS SumElecBen
        ,SUM(GasBen) AS SumGasBen
        ,SUM(ElecBenGross) AS SumElecBenGross
        ,SUM(GasBenGross) AS GasBenGross
    FROM #OutputCE
    GROUP BY PrgID
)
, BenPos (
