...
--------------------------------------------------------------------------------
    ...
    -- Get fractional generation (Genfrac) avoided costs for water energy savings. Fractions are effective useful lives (eul) for fractions of a quarter. First Baseline fraction
        ...
        AS A GROUP BY CET_ID
    ) AS acw_1 ON e.CET_ID=acw_1.CET_ID
    ...
    SELECT
        ...
        SUM(
            (
                Qty *
                CASE
                    WHEN e.MeasImpactType LIKE '%FuelSub' OR e.Channel = 'C&S' OR (e.IncentiveToOthers + e.DILaborCost + e.DIMaterialCost - e.UnitMeasureGrossCost > 0)
