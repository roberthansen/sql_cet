            ...
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN
                            Qty * (IRkWh * RRkWh * kWhWater1 * RUL + IRkWh * RRkWh * kWhWater2 * (EUL - RUL)) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
        ...
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * (NTGRkWh+@MEBens) * (IRkWh * RRkWh *  kWhWater1 * RUL + IRkWh * RRkWh *  kWhWater2 * (EUL - RUL)) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IRkWh * RRkWh * (NTGRkWh+@MEBens) * kWhWater1
                        ELSE 0
                    END
            END
        ) AS NetkWhWater
...
--*****************  Lifecycle Gross   *****************************************
        ,SUM(
           CASE
                WHEN IsNull(RUL,0) = 0 AND EUL >= 1
                THEN Qty * EUL * IRkWh * RRkWh * kWh1
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * EUL * IRkWh * RRkWh * (kWh1*Isnull(RUL,0) + kWh2*(EUL-Isnull(RUL,0)))/EUL
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * EUL * IRkWh * RRkWh * kWh1
                                ELSE Qty * IRkWh * RRkWh * (RUL * kWh1 + EUL * kWh2)
                            END
                    END
            END
        ...
        ,SUM(
            CASE
                ...
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * EUL * IRThm * RRThm * (Thm1*Isnull(RUL,0) + Thm2*(EUL-Isnull(RUL,0)))/EUL 
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * EUL * IRThm * RRThm * Thm1
                                ELSE Qty * IRThm * RRThm * (RUL * Thm1 + EUL * Thm2)
                            END
                    END
            END
        ...
        ,SUM(
           CASE
                ...
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * EUL * IRkWh * RRkWh * (kWhWater1*Isnull(RUL,0) + kWhWater2*(EUL-Isnull(RUL,0)))/EUL
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * EUL * IRkWh * RRkWh * kWhWater1
                                ELSE Qty * IRkWh * RRkWh * (RUL * kWhWater1 + EUL * kWhWater2)
                            END
                    END
            END
        ...
        ,SUM(
            CASE
                ...
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * (NTGRkWh+@MEBens) * EUL * IRkWh * RRkWh * (kWh1*Isnull(RUL,0) + kWh2*(EUL-Isnull(RUL,0)))/EUL
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * (NTGRkWh+@MEBens) * EUL * IRkWh * RRkWh * kWh1
                                ELSE Qty * (NTGRkWh+@MEBens) * IRkWh * RRkWh * (RUL * kWh1 + EUL * kWh2)
                            END
                    END
            END
        ) AS LifecycleNetkWh
        ,SUM(
            CASE
                ...
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * (NTGRThm+@MEBens) * EUL * IRThm * RRThm * (Thm1*Isnull(RUL,0) + Thm2*(EUL-Isnull(RUL,0)))/EUL
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * (NTGRThm+@MEBens) * EUL * IRThm * RRThm * Thm1
                                ELSE Qty * (NTGRThm+@MEBens) * IRThm * RRThm * (RUL * Thm1 + EUL * Thm2)
                            END
                    END
            END
        ...
        ,SUM(
            CASE
                ...
                ELSE
                    CASE
                        WHEN IsNull(RUL,0) > 0 AND EUL >= 1
                        THEN Qty * (NTGRkWh+@MEBens) * EUL * IRkWh * RRkWh * (kWhWater1*Isnull(RUL,0) + kWhWater2*(EUL-Isnull(RUL,0)))/EUL
                        ELSE
                            CASE
                                WHEN IsNull(RUL,0) = 0 AND EUL < 1
                                THEN Qty * (NTGRkWh+@MEBens) * EUL * IRkWh * RRkWh * kWhWater1
                                ELSE Qty * (NTGRkWh+@MEBens) * IRkWh * RRkWh * (RUL * kWhWater1 + EUL * kWhWater2)
                            END
                    END
            END
...
--*****************  First Year Gross Savings   ********************************
    ,SUM(
        ...
        + CASE
            ...
            THEN EUL - RUL
            WHEN RUL > 0 AND RUL <= 1 AND (RUL + EUL) >= 1
            THEN 1 - RUL
            ELSE 0
