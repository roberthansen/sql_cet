...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.CalcCost'))
   exec('CREATE PROCEDURE [dbo].[CalcCost] AS BEGIN SET NOCOUNT ON; END')
GO

ALTER PROCEDURE [dbo].[CalcCost]
@JobID INT = -1,
@MEBens FLOAT=NULL,
@MECost FLOAT=NULL,
@FirstYear INT,
...
(
	SELECT CE.CET_ID
		...
		, @MECost * ((ri.GrossMeasCost) + (ex.ExcessIncentivesPV)) AS MarkEffectPlusExcessInc
...
(
	SELECT e.CET_ID
		...
		,CASE WHEN IsNull(RUL,0) >0 THEN
			CASE WHEN kWh1 <> 0 THEN CASE WHEN Power(Rqf, Qm) <> 0 THEN CASE WHEN Ra <> 0 THEN CASE WHEN Power(Raf, eul-1) <> 0 THEN CASE WHEN IsNull(eul,0) <> 0 THEN Sum((NTGRkWh+@MEBens) * Qty * ((IRkWh * RRkWh * kwh1 * (1 + (1-1/(Power(Raf, rul-1)))/Ra)) + (IRkWh * RRkWh * kwh2 * (1 + (1-1/(Power(Raf, (eul-rul)-1)))/Ra))/Power(Raf, rul))/ Power(Rqf, Qm)) ELSE 0 END END END END END
		ELSE
			CASE WHEN kWh1 <> 0 THEN CASE WHEN Power(Rqf, Qm) <> 0 THEN CASE WHEN Ra <> 0 THEN CASE WHEN Power(Raf, eul-1) <> 0 THEN CASE WHEN IsNull(eul,0) <> 0 THEN Sum((NTGRkWh+@MEBens) * Qty * ((IRkWh * RRkWh * kwh1 * (1 + (1-1/(Power(Raf, eul-1)))/Ra)))/ Power(Rqf, Qm)) ELSE 0 END END END END END END as DiscountedSavingsGrosskWh
		...
		,CASE WHEN IsNull(RUL,0) >0 THEN
			CASE WHEN Thm1 <> 0 THEN CASE WHEN Power(Rqf, Qm) <> 0 THEN CASE WHEN Ra <> 0 THEN CASE WHEN Power(Raf, eul-1) <> 0 THEN CASE WHEN IsNull(eul,0) <> 0 THEN Sum((NTGRThm+@MEBens) * Qty * ((IRThm * RRThm * Thm1 * (1 + (1-1/(Power(Raf, rul-1)))/Ra)) + (IRThm * RRThm * Thm2 * (1 + (1-1/(Power(Raf, (eul-rul)-1)))/Ra))/Power(Raf, rul))/ Power(Rqf, Qm)) ELSE 0 END END END END END
		ELSE
