        ...
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (kWh1 * Isnull(RUL, 0) + kWh2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
        ...
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (kW1 * Isnull(RUL, 0) + kW2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
        ...
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (Thm1 * Isnull(RUL, 0) + Thm2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
        ...
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR *  (NTGRkWh+@MEBens) * (kWh1 * Isnull(RUL, 0) + kWh2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (NTGRkWh+@MEBens) * kWh1
                        ELSE 0
                    END
            END
        ) AS NetkWh
        ,SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (NTGRkW+@MEBens) * (kW1 * Isnull(RUL, 0) + kW2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (NTGRkW+@MEBens) * kW1
                        ELSE 0
                    END
            END
        ) AS NetkW
        , SUM(
            CASE
                ...
                THEN
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (NTGRThm+@MEBens) * (Thm1 * Isnull(RUL, 0) + Thm2 * (EUL - Isnull(RUL, 0))) / EUL
                        ELSE 0
                    END
                ELSE
                    CASE
                        WHEN EUL > 0
                        THEN Qty * IR * RR * (NTGRThm+@MEBens) * Thm1
                        ELSE 0
                    END
            END
        ) AS NetTherms
...
/*
--------------------------------------------------------------------------------
--- First Year Gross (obsolete) ------------------------------------------------
    ,SUM(
        CASE
            WHEN IsNull(RUL, 0) >= 1 THEN 1
            WHEN IsNull(RUL, 0) > 0 AND IsNull(RUL, 0) <= 1 THEN RUL
            WHEN RUL = 0 AND EUL >= 1 THEN 1
            ELSE 1
        END * Qty * IRkWh * RRkWh * kWh1 +
        CASE
            WHEN RUL > 0 AND RUL <= 1 THEN 1 - RUL
            ELSE 0
        END * Qty * IRkWh * RRkWh * kWh2
    ) AS FirstYearGrosskWh
