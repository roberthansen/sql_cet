...
--- PVBenNet[E]: Present value net electricity benefits
        ,SUM(
            CASE
                WHEN e.MeasImpactType NOT LIKE '%FuelSub' OR e.MeasImpactType NOT LIKE '%NC-AE' OR e.kWh1>0
                THEN
                    ISNULL(
                        e.Qty *
                        (
...
--- NetPVBenTOT[G]: Present value of net gas benefits
        ,SUM(
            CASE 
                WHEN e.MeasImpactType NOT LIKE '%FuelSub' OR e.MeasImpactType NOT LIKE '%NC-AE' OR e.Thm1>0
                THEN
                    ISNULL(
                        e.Qty *
                        ( e.NTGRThm + @MEBens ) *
            ...
            CASE
                ...
                THEN
                    ISNULL(
                        ...
                        e.RRkWh *
                        (
                            e.kWhWater1 * acw_1.Gen +
                            e.kWhWater2 * acw_2.Gen
                        ),
                        0
                    )
                ELSE 0
...
--------------------------------------------------------------------------------
    ...
    LEFT JOIN (
        SELECT
            ...
            ,SUM(Gen) AS Gen
        FROM (
    -- Get Generation (Gen) avoided costs for water energy savings
            SELECT
                CET_ID
                ,ISNULL( Gen_Water / POWER( Rqf, Qac ), 0 ) AS Gen
            FROM AvoidedCostElecvw
            WHERE Qac BETWEEN Qm AND Qm + CONVERT( INT, eulq1 ) - 1

    -- Get fractional generation (Genfrac) avoided costs for water energy savings. Fractions are effective useful lives (eul) for fractions of a quarter. First Baseline fraction
            UNION SELECT
                CET_ID
                ,( eulq1 - FLOOR( eulq1 ) ) * (Gen_Water / POWER( Rqf, Qac ) ) AS Gen
            FROM AvoidedCostElecvw
            WHERE Qac = Qm + CONVERT( INT, eulq1 )
        )
        AS A GROUP BY CET_ID
    ) AS acw_1
    ON e.CET_ID=acw_1.CET_ID
    ...
    LEFT JOIN (
        SELECT
            ...
            ,SUM(Gen) AS Gen
        FROM (
    -- Get fractional generation (Genfrac) avoided costs for water energy savings. Second baseline fraction.
            SELECT
                CET_ID
                ,CASE
                    WHEN eulq2 > 0 AND ( eulq1 - FLOOR( eulq1 ) ) > 0
                    THEN ( 1 - ( eulq1 - FLOOR( eulq1 ) ) ) * ( Gen_Water / POWER( Rqf, Qac ) )
                    ELSE 0
                END AS Gen
            FROM AvoidedCostElecvw
            WHERE Qac = Qm + CONVERT( INT, eulq1 )
    ...
    -- Get fractional generation (Genfrac) and transmission & Distribution (TDfrac) avoided costs. Fractions are effective useful lives (eul) for fractions of a quarter
            UNION SELECT
                CET_ID
                ,( eulq2 - ROUND( eulq2, 0, 1 ) ) * ( Gen_Water / POWER( Rqf, Qac ) ) AS Gen
            FROM AvoidedCostElecvw
            WHERE Qac = Qm + CONVERT( INT, eulq2 )
        ) AS A GROUP BY CET_ID, DS/*, DS_AL*/
    ) AS acw_2
    ON e.CET_ID = acw_2.CET_ID
...
(
    SELECT
        ...
        ,CASE WHEN ISNULL(ElecBen,0) > 0 THEN ElecBen ELSE 0 END +
          ...
          CASE WHEN ISNULL(WaterEnergyBen) > 0 THEN WaterEnergyBen ELSE 0 END +
                    ...
                    (
                       ...
                       CASE 
                        ...
                        CASE
                            ...
                            ELSE 0
                        END
                    ) / (BensSum.SumElecBen + BensSum.SumGasBen + BensSum.SumElecBen + BensSum.SumOtherBen)
                ELSE
                    -- If no benefits then divide program costs evenly among claims
                    1.000 / cc.ClaimCount
            END *
