...
PRINT 'Inserting electrical and gas benefits... Message 3'
    ...
    SELECT @JobID 
        ...
        ,e.PrgID
        ,e.CET_ID
        ,ISNULL(SUM(POWER(Rqf, Qm - 1) * (Qty * (((NTGRkWh+COALESCE(e.MEBens,@MEBens)) * IRkWh * RRkWh * kWh1 * (Gen1 + Genfrac1) + IRkW * RRkW * (e.NTGRkw+COALESCE(e.MEBens,@MEBens)) * DS1 * (TD1 + TDfrac1)) + IRkWh * RRkWh * ((NTGRkWh+COALESCE(e.MEBens,@MEBens)) * kWh2 * (ISNULL(Gen2,0) + ISNULL(Genfrac2_1,0) + ISNULL(Genfrac2_2,0)) + (e.NTGRkw+COALESCE(e.MEBens,@MEBens)) * ISNULL(DS2,0) * (ISNULL(TD2,0) + ISNULL(TDfrac2_1,0) + ISNULL(TDfrac2_2,0))))/ POWER(Rqf, Qm))),0) -- First and second baselines
        AS ElecBen
        ,ISNULL(SUM(POWER(Rqf, Qm - 1) * (Qty * (((e.NTGRThm+COALESCE(e.MEBens,@MEBens)) * IRThm * RRThm * Thm1 * (Gas1 + ISNULL(Gasfrac1, 0))) + IRThm * RRThm * ((e.NTGRThm+COALESCE(e.MEBens,@MEBens)) * Thm2 * (ISNULL(Gas2,0) + ISNULL(Gasfrac2_1, 0) + ISNULL(Gasfrac2_2, 0))))/ POWER(Rqf, Qm))),0) -- First and second baselines
        AS GasBen
        ,ISNULL(SUM(POWER(Rqf, Qm - 1) * (Qty * (((1+COALESCE(e.MEBens,@MEBens)) * IRkWh * RRkWh * kWh1 * (Gen1 + Genfrac1) + DS1 * (TD1 + TDfrac1)) + (IRkWh * RRkWh * kWh2 * (ISNULL(Gen2,0) + ISNULL(Genfrac2_1,0) + ISNULL(Genfrac2_2,0)) + ISNULL(DS2,0) * (ISNULL(TD2,0) + ISNULL(TDfrac2_1,0) + ISNULL(TDfrac2_2,0))))/ POWER(Rqf, Qm))),0) -- First and second baselines
        AS ElecBenGross
        ,ISNULL(SUM(POWER(Rqf, Qm - 1) * (Qty * (((1+COALESCE(e.MEBens,@MEBens)) * IRThm * RRThm * Thm1 * (Gas1 + ISNULL(Gasfrac1, 0))) + (IRThm * RRThm * Thm2 * (ISNULL(Gas2,0) + ISNULL(Gasfrac2_1, 0) + ISNULL(Gasfrac2_2, 0))))/ POWER(Rqf, Qm))),0) -- First and second baselines
        AS GasBenGross
